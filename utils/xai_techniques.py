...
def generate_integrated_gradients(model, input_tensor):
    ...
    try:
        ...
        attribution_map = integrated_gradients.attribute(input_tensor, baseline, target=pred_index)
...
def generate_occlusion_sensitivity(model, device, input_tensor):
    ...
    try:
        ...
        input_tensor_cpu = input_tensor.to('cpu') # Captum occlusion works well on CPU
